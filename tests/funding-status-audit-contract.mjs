...
assert.equal(require("../package.json").scripts.prebuild, "npm run test:funding-status && npm run audit:funding-status && npm run test:brand-entity && npm run test:trust-evidence && npm run test:contact-nap");
