...
assert.equal($("#hero.im-hero").attr("data-homepage-revision"), "immersive-20260902-2", "build-ul trebuie să păstreze designul imersiv curent");
...
assert.equal(verifyHomepageContent(html, html).revision, "immersive-20260902-2");
assert.equal(verifyHomepageContent(html, html).assets.length, 9);
assert.throws(() => verifyHomepageContent(html.replace("immersive-20260902-2\"", "old\""), html));
...
assert.throws(() => verifyHomepageContent(html.replace("homepage-decision-flow.js?v=20260902-2", "homepage-decision-flow.js?v=20260901-1"), html));
