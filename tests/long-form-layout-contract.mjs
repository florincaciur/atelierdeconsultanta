...
for (const page of report.pages) {
  ...
  assert.equal($("[data-long-form-toc]").length, 1, `${page.route}: trebuie un singur cuprins`);
  ...
  links.each((_, link) => {
    ...
    assert.equal($(href).length, 1, `${page.route}: ținta ${href} lipsește sau este duplicată`);
  });
...
assert(homepage.indexOf("<!-- HOMEPAGE_DECISION_HERO_END -->") < homepage.indexOf("<!-- P1_09_LONG_FORM_TOC_START -->"), "cuprinsul homepage-ului trebuie să urmeze întregului bloc hero");
