...
for (const page of report.pages) {
  ...
  const toc = page.route === "/" ? $("#navbar [data-homepage-navbar-toc]") : $("[data-long-form-toc]");
  assert.equal(toc.length, 1, `${page.route}: trebuie un singur cuprins`);
  ...
  if (page.route === "/") {
    assert.equal(toc.children("button").clone().children().remove().end().text().trim(), "Cuprins", `${page.route}: butonul navbar nu are etichetă`);
    assert.equal(toc.children("button").attr("aria-expanded"), "false", `${page.route}: cuprinsul navbar trebuie să pornească închis`);
  } else {
  ...
  const links = page.route === "/" ? toc.find("[data-homepage-toc-link]") : toc.find("[data-long-form-toc-link]");
  ...
  links.each((_, link) => {
    ...
    assert(page.route === "/" ? href?.startsWith("/#") : href?.startsWith("#"), `${page.route}: linkul de cuprins nu este ancoră`);
...
assert(homepage.indexOf("data-homepage-navbar-toc") < homepage.indexOf("<!-- HOMEPAGE_DECISION_HERO_START -->"), "cuprinsul homepage-ului trebuie să fie în navbar, înainte de hero");
