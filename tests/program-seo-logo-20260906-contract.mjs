...
const compact = cheerio.load(fs.readFileSync(path.join(BASE, 'assets/faber-navbar-vector-compact.svg'), 'utf8'), { xmlMode: true });
for (const svg of [wide, compact]) {
  assert.equal(svg('path').length, 3, 'Trace includes monogram, FABER and complete descriptor');
  assert.equal(svg('image, text, script, foreignObject').length, 0, 'Vector paths only: no raster, font substitution or scripts');
  assert.equal(svg('#monogram').attr('fill'), '#f5a623');
