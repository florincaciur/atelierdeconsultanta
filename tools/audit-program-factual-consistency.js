...
function auditNarrativeClaims(issues, program, $) {
  ...
  if (openClaim && program.sourceStatus !== "open") {
    addIssue(issues, program, "error", "status-mismatch", "page-content", "Pagina afirmă că apelul este deschis, dar registrul nu are sourceStatus=open.", program.sourceStatus, openClaim);
  }
}
...
function auditHeader(issues, programs, header) {
  ...
  for (const program of programs) {
    ...
    compare(issues, program, "status-mismatch", "navbar-desktop", program.sourceStatus, desktop.attr("data-source-status"), "Statusul din navbar diferă de config.");
    compare(issues, program, "freshness-mismatch", "navbar-desktop", program.reviewedAt, desktop.attr("data-reviewed-at"), "Data de verificare din navbar diferă de config.");
    compare(issues, program, "label-mismatch", "navbar-desktop", program.menuSecondaryLabel, desktop.find(".d-label").text(), "Eticheta secundară din navbar diferă de config.");
    compare(issues, program, "label-mismatch", "navbar-mobile", program.menuLabel, mobile.text(), "Eticheta mobilă diferă de config.");
    const menuFinancial = financialClaimTokens(desktop.find(".d-label").text());
    if (program.sourceStatus === "consultation" && menuFinancial.size) {
...
function auditProgram(issues, program, context) {
  ...
  if (pageConfig) {
    ...
    compare(issues, program, "meta-mismatch", "page-config", program.metaTitle, pageConfig.title, "Title-ul din pagina configurată diferă de registru.");
    compare(issues, program, "meta-mismatch", "page-config", program.metaDescription, pageConfig.description, "Meta-description din pagina configurată diferă de registru.");
  ...
  if (!guide) addIssue(issues, program, "error", "missing-source", "official-guides.json", "Lipsește sursa oficială principală asociată programului.", primaryKey, "absent");
    ...
    compare(issues, program, "status-mismatch", "official-guides.json", program.sourceStatus, guide.sourceStatus, "Statusul sursei diferă de registru.");
    compare(issues, program, "freshness-mismatch", "official-guides.json", program.reviewedAt, guide.reviewedAt || guide.accessedAt, "Data verificării sursei diferă de registru.");
  }

  const banner = bannersByProgram.get(program.id);
  if (banner) {
    compare(issues, program, "status-mismatch", "banners.json", program.sourceStatus, banner.sourceStatus, "Statusul bannerului diferă de config.");
    compare(issues, program, "amount-mismatch", "banners.json", fundingSummary(program), banner.amount, "Rezumatul financiar al bannerului diferă de config.");
    ...
    compare(issues, program, "source-mismatch", "banners.json", program.officialSourceUrl, banner.officialSourceUrl, "Sursa bannerului diferă de config.");
  }
  ...
  if (llmsIndex === -1) addIssue(issues, program, "error", "missing-llms-entry", "llms.txt", "Programul lipsește din llms.txt.", url, "absent");
    ...
    if (!comparable(llmsFragment).includes(comparable(statusLabel(program.sourceStatus)))) addIssue(issues, program, "error", "status-mismatch", "llms.txt", "Statusul programului lipsește sau diferă în llms.txt.", statusLabel(program.sourceStatus), llmsFragment);
  }

  if (ACTIVE_STATUSES.has(program.sourceStatus) && daysSince(program.reviewedAt) > 30) {
  ...
  compare(issues, program, "status-mismatch", "page-html", program.sourceStatus, $("body").attr("data-source-status"), "Statusul paginii HTML diferă de config.");
  compare(issues, program, "freshness-mismatch", "page-html", program.reviewedAt, $("body").attr("data-reviewed-at"), "Data paginii HTML diferă de config.");
  ...
  if (!note.length) addIssue(issues, program, "error", "missing-factual-status", "page-html", "Lipsește nota factuală vizibilă sincronizată.");
    ...
    compare(issues, program, "status-mismatch", "page-status", program.sourceStatus, note.attr("data-source-status"), "Nota factuală are alt status.");
    ...
    for (const required of [statusStatement(program), fundingSummary(program), program.factualDisclaimer]) {
    ...
    if (program.sourceStatus === "consultation" && !comparable(noteText).includes("conform variantei consultative")) {
    ...
    if (program.sourceStatus === "closed" && note.find("a[href^='/']").length === 0) {
  ...
  if (!programNode) addIssue(issues, program, "error", "missing-program-jsonld", "json-ld", "Lipsește entitatea factuală DefinedTerm a programului.");
    ...
    compare(issues, program, "status-mismatch", "json-ld", program.sourceStatus, properties.get("sourceStatus"), "Statusul JSON-LD diferă de config.");
    compare(issues, program, "freshness-mismatch", "json-ld", program.reviewedAt, properties.get("reviewedAt"), "Data JSON-LD diferă de config.");
  }
...
function writeReports(programs, issues) {
  ...
  const statusCounts = Object.fromEntries([...new Set(programs.map((program) => program.sourceStatus))].sort().map((status) => [status, programs.filter((program) => program.sourceStatus === status).length]));
  ...
  const statusRows = programs.map((program) => `| ${program.shortName} | ${program.sourceStatus} | ${program.reviewedAt} | ${program.applicationStart || "—"} | ${program.applicationEnd || "—"} |`).join("\n");
  ...
  const headers = ["programId", "route", "auditResult", "sourceStatus", "reviewedAt", "applicationStart", "applicationEnd", "officialSourceUrl", "maximumGrant", "minimumGrant", "intensity", "errorCount", "warningCount", "issueCategories"];
  const csvRows = programs.map((program) => {
    ...
    return {
      ...
      maximumGrant: formatMoneyValue(program.maximumGrant),
      minimumGrant: formatMoneyValue(program.minimumGrant),
      intensity: formatIntensity(program.intensity),
      errorCount,
      warningCount,
